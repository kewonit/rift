...
|---|---|---|---|---|
| GRDB.swift | 7.10.0 / `36e30a6f1ef10e4194f6af0cff90888526f0c115` | MIT | App-only configuration/history repositories | Replace `AbyssControl` repository implementation |
| Swift Argument Parser | 1.8.2 / `6a52f3251125d74daf04fcbd5e6f08a75d074382` | Apache-2.0 with Runtime Library Exception | Embedded `abyssctl` argument parsing only | Replace the CLI command front end; IPC/file-safety code is separate |

The Abyss maintainers own dependency review. Before each release candidate—and
...
There are no shipping UI, chart, map, update, or geolocation dependencies. Swift Charts, AppKit/SwiftUI, NetworkExtension, Security, CryptoKit, ServiceManagement, UserNotifications, and SystemConfiguration are system frameworks. No third-party blocklist or geolocation data ships. GRDB's optional SQLCipher/plugin dependencies are not enabled. The workspace lock pins both remote packages, and the standalone `AbyssControl` package lock independently pins GRDB; neither dependency floats on a branch.
...
is not shipped in Abyss. The workflow runs on the standard Apple-silicon
