...
Rift is designed to make firewall decisions on the Mac. It has no analytics,
...
- Rule-use counters are separate from connection history and can be cleared
  separately.
- Notifications are private by default. Rift never includes complete URLs or
...
- Application names and icons are read only from currently installed local apps.
  ...
  icon.
- If the user manually imports DB-IP City Lite, Rift stores the validated IP
...
requests map tiles for the coordinate regions currently in view. Rift supplies
...
Exported files leave Rift's control. Review them before sharing and use a secure
