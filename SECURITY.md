...
Rift is fail-open when no compatible validated policy is available. A root attacker is outside the integrity claim. Same-user database tampering after a sandbox/DAC compromise may alter editable configuration, but should not directly write the extension's root-owned durable policy cache.
